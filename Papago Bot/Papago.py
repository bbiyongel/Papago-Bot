...
async def 도움말(ctx):
    embed=discord.Embed(title="「도움말」", description=helpdescription, color=color)
...
async def 번역(ctx, language1, language2, *, text):
    ...
    embed=discord.Embed(title=f"「번역 결과 {language1} -> {language2}」", description=f"{ctx.author.display_name }: {result_text}", color=color)
    embed.set_thumbnail(url=papagoimage)
    embed.set_footer(text=API)
...
async def 자동번역(ctx, language, *, text):
    ...
    result = response.json()
    result_text = result["message"]["result"]["translatedText"]
    embed=discord.Embed(title=f"「번역 결과 {detect_lang} -> {language}」", description=f"{ctx.author.display_name }: {result_text}", color=color) 
    embed.set_thumbnail(url=papagoimage)
    embed.set_footer(text=API)
    await ctx.send(embed=embed)
...
@bot.command(pass_context=True)
async def 사운드번역(ctx, language, *, text):
    detect_text = {"query": text}
    detect_response = rq.post(detect_url, headers=headers, data=detect_text)
    detect_result = detect_response.json()
    detect_lang = detect_result["langCode"]
    ...
    result = response.json()
    result_text = result["message"]["result"]["translatedText"]
    out = gTTS(text=result_text, lang=language, slow=False)
...
@bot.command(pass_context=True)
async def 번역언어(ctx):
    embed=discord.Embed(title="「번역 언어」", description=language_ko, color=color)
    embed.set_thumbnail(url=papagoimage)
    embed.set_footer(text=API)
    await ctx.author.send(embed=embed)
...
@bot.command(pass_context=True)
async def API목록(ctx):
    embed=discord.Embed(title=f"「API」", description=APIdescription, color=color)
    embed.set_thumbnail(url=papagoimage)
    embed.set_footer(text=API)
    await ctx.author.send(embed=embed)
